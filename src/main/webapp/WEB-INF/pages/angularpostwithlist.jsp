...
<script	src="//netdna.bootstrapcdn.com/bootstrap/3.1.1/js/bootstrap.min.js"></script>
   ...
   <div ng-controller="AngularPostController as personCtrl" ng-app="myApp" class="container"  align="center"  >    
		...
		<tr><td colspan="2">
		  ...
		  <div ng-if="personCtrl.flag == 'edit'">
		     <h3> Update Post for ID: {{ personCtrl.person.id }}
		      Update Post for User: {{ personCtrl.person.postUser }} </h3> 
		  </div> </td>
		</tr>		
	<!-- 	<tr>
		<td> Category</td>
	<td><select ng-model="personCtrl.person.category" ng-options="x for x in names"></select></td>
		...
		<tr>
	        ...
	        <td colspan="2">
		       ...
		       <input  type="submit" ng-click="personCtrl.addPerson()" value="Add Post"/> 
  ...
  <select ng-model="pageSize" id="pageSize" class="form-control">
    ...
    <tbody>
	       <tr ng-repeat="row in personCtrl.persons ">
		     ...
		     <div ng-if = "${(pageContext.request.userPrincipal.name == 'admin') || (pageContext.request.userPrincipal.name == row.postUser)}">
		      {{row.postUser}}     ||    ${pageContext.request.userPrincipal.name}
		     ...
		     <div ng-if= "${pageContext.request.userPrincipal.name == row.postUser}">
		      {{row.postUser}}     ||    ${pageContext.request.userPrincipal.name}
